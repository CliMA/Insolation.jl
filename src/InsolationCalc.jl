...
"""
    solar_flux(
        ...
        solar_variability::Union{TSIDataSpline,Nothing} = nothing,
    ) where {FT<:Real}
...
- `date::Union{DateTime,Nothing}`: (default `nothing`) Current date and time to evaluate the
  solar flux if `tsi_spline` is not `nothing`
- `solar_variability::Union{TSIDataSpline,Nothing}`: Use time-varying solar luminosity if
...
function solar_flux(
    ...
    S0::FT =
        isnothing(solar_variability) ? IP.tot_solar_irrad(param_set) :
        FT(evaluate(solar_variability, date))
...
function insolation(
    ...
    S = solar_flux(d, param_set, date, solar_variability)
...
- `solar_variability::Union{TSIDataSpline, Nothing}`: (default nothing) Use time-varying
...
cpu_solar = TSIDataSpline() # Create on CPU
...
function insolation(
    ...
    F, S, μ = insolation(θ, d, param_set, date, solar_variability)
...
function daily_insolation(
    ...
    F, S, μ = insolation(daily_θ, d, param_set, date, solar_variability)
